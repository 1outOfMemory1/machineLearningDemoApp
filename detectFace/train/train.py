...
if __name__ == '__main__':
    ...
    parser.add_argument('--epochs', type=int, default=30)  # 训练轮数
    parser.add_argument('--batch-size', type=int, default=16)  # batch-size
    parser.add_argument('--cfg', type=str, default='cfg/my_yolov3.cfg', help="*.cfg path") # 网络结构
    parser.add_argument('--data', type=str, default='data/my_data.data', help='*.data path')  # 种类文件
    parser.add_argument('--hyp', type=str, default='cfg/hyp.yaml', help='hyperparameters path')  # 超参数 除非是炼丹大师否则不要动
    parser.add_argument('--multi-scale', type=bool, default=True,
                        help='adjust (67%% - 150%%) img_size every 10 batches')
    parser.add_argument('--img-size', type=int, default=512, help='test size')   # 训练时图像大小
    ...
    parser.add_argument('--weights', type=str, default='weights/yolov3spp-14.pt',   # 初始训练的时候的权重文件存放地址
                        help='initial weights path')
    parser.add_argument('--name', default='', help='renames results.txt to results_name.txt if supplied')
    parser.add_argument('--device', default='cuda:0', help='device id (i.e. 0 or 0,1 or cpu)')  # 训练时选择的 默认是显卡
    parser.add_argument('--single-cls', action='store_true', help='train as single-class dataset')
    parser.add_argument('--freeze-layers', type=bool, default=False, help='Freeze non-output layers')  # 是否要冻结前边的层
    ...
    print('Start Tensorboard with "tensorboard --logdir=runs", view at http://localhost:6006/')  #tensorBoard可以将loss等参数进行可视化
